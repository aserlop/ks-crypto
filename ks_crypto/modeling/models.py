...
from ks_crypto.lib import modeling_utils as mu
...
class Classifier(torch.nn.Module):
    def __init__(self, args_dic, out_features=2, in_features=None):
        ...
        if in_features is not None:
            ...
            num_feats = args_dic['layer_2_feats'] * 2

        self.mlp = torch.nn.Sequential(torch.nn.Linear(in_features=num_feats,
                                                       out_features=args_dic['cls_feats']),
                                       activation,
                                       torch.nn.Linear(in_features=args_dic['cls_feats'],
