...
class Logger:
    def __init__(self, args_dic, num_classes, logging, minibatch_log_interval=10):

        self.logging = logging

        self.logging.info('*** PARAMETERS ***')
        self.logging.info(pprint.pformat(args_dic))
        self.logging.info('')
    ...
    def log_epoch_start(self, epoch, num_minibatches, set, minibatch_log_interval=None):
        ...
        if minibatch_log_interval is not None:
            self.minibatch_log_interval = minibatch_log_interval
        self.logging.info('################ ' + set + ' epoch ' + str(epoch) + ' ###################')
        self.lasttime = time.monotonic()
        self.ep_time = self.lasttime

    def log_minibatch(self, predictions, true_classes, loss):
        ...
        if self.minibatch_done % self.minibatch_log_interval == 0:
            ...
            self.logging.info(
                self.set + 'batch %d / %d - partial error %0.4f - partial loss %0.4f - partial MRR  %0.4f - partial '
                           'MAP %0.4f' % (
                    self.minibatch_done, self.num_minibatches, mb_error, partial_losses.mean(), mb_MRR, mb_MAP))

            tp = conf_mat_per_class['true_positives']
            fn = conf_mat_per_class['false_negatives']
            fp = conf_mat_per_class['false_positives']
            self.logging.info(self.set + ' batch %d / %d -  partial tp %s,fn %s,fp %s' % (
                self.minibatch_done, self.num_minibatches, tp, fn, fp))
            precision, recall, f1 = self.calc_microavg_eval_measures(tp, fn, fp)
            self.logging.info(
                self.set + ' batch %d / %d - measures partial microavg - precision %0.4f - recall %0.4f - f1 %0.4f ' % (
            ...
            for cl in range(self.num_classes):
                ...
                self.logging.info(
                    self.set + 'batch %d / %d - measures partial for class %d - precision %0.4f - recall %0.4f - f1 '
                               '%0.4f ' % (
                        self.minibatch_done, self.num_minibatches, cl, cl_precision, cl_recall, cl_f1))

            self.logging.info(self.set + ' batch %d / %d - Batch time %d ' % (
    ...
    def log_epoch_done(self):
        ...
        self.logging.info(self.set + ' mean losses ' + str(self.losses.mean()))
        if self.args_dic['target_measure'] == 'loss' or self.args_dic['target_measure'] == 'Loss':
            eval_measure = self.losses.mean()

        epoch_error = self.calc_epoch_metric(self.batch_sizes, self.errors)
        self.logging.info(self.set + ' mean errors ' + str(epoch_error))

        epoch_MRR = self.calc_epoch_metric(self.batch_sizes, self.MRRs)
        epoch_MAP = self.calc_epoch_metric(self.batch_sizes, self.MAPs)
        self.logging.info(self.set + ' mean MRR ' + str(epoch_MRR) + ' - mean MAP ' + str(epoch_MAP))
        if self.args_dic['target_measure'] == 'MRR' or self.args_dic['target_measure'] == 'mrr':
            eval_measure = epoch_MRR
        if self.args_dic['target_measure'] == 'MAP' or self.args_dic['target_measure'] == 'map':
            eval_measure = epoch_MAP

        self.logging.info(self.set + ' tp %s,fn %s,fp %s' % (self.conf_mat_tp, self.conf_mat_fn, self.conf_mat_fp))
        precision, recall, f1 = self.calc_microavg_eval_measures(self.conf_mat_tp, self.conf_mat_fn, self.conf_mat_fp)
        self.logging.info(
            self.set + ' measures microavg - precision %0.4f - recall %0.4f - f1 %0.4f ' % (precision, recall, f1))
        if str(self.args_dic['target_class']) == 'AVG':
            if self.args_dic['target_measure'] == 'Precision' or self.args_dic['target_measure'] == 'prec':
                eval_measure = precision
            elif self.args_dic['target_measure'] == 'Recall' or self.args_dic['target_measure'] == 'rec':
                eval_measure = recall
            else:
                eval_measure = f1

        for cl in range(self.num_classes):
            cl_precision, cl_recall, cl_f1 = self.calc_eval_measures_per_class(self.conf_mat_tp, self.conf_mat_fn,
                                                                               self.conf_mat_fp, cl)
            self.logging.info(self.set + ' measures for class %d - precision %0.4f - recall %0.4f - f1 %0.4f ' % (
                cl, cl_precision, cl_recall, cl_f1))
            if str(cl) == str(self.args_dic['target_class']):
                if self.args_dic['target_measure'] == 'Precision' or self.args_dic['target_measure'] == 'prec':
        ...
        for k in self.eval_k_list:
            precision, recall, f1 = self.calc_microavg_eval_measures(self.conf_mat_tp_at_k[k], self.conf_mat_fn_at_k[k],
                                                                     self.conf_mat_fp_at_k[k])
            self.logging.info(self.set + ' measures@%d microavg - precision %0.4f - recall %0.4f - f1 %0.4f ' % (
                k, precision, recall, f1))

            for cl in range(self.num_classes):
                cl_precision, cl_recall, cl_f1 = self.calc_eval_measures_per_class(self.conf_mat_tp_at_k[k],
                                                                                   self.conf_mat_fn_at_k[k],
                                                                                   self.conf_mat_fp_at_k[k], cl)
                self.logging.info(
                    self.set + ' measures@%d for class %d - precision %0.4f - recall %0.4f - f1 %0.4f ' % (
                        k, cl, cl_precision, cl_recall, cl_f1))

        self.logging.info(self.set + ' Total epoch time: ' + str((time.monotonic() - self.ep_time)))
    ...
    def eval_predicitions(self, predictions, true_classes, num_classes):
        ...
        conf_mat_per_class = {'true_positives': {}, 'false_negatives': {}, 'false_positives': {}}
    ...
    def eval_predicitions_at_k(self, predictions, true_classes, num_classes, k):
        ...
        for cl in range(num_classes):
            ...
            fn = true_classes[cl_indices].size(
                0) - tp  # This only if we want to consider the size at K -> hits.size(0) - tp
            fp = pos.sum() - tp
